...
rfo.prototype.child = function(selector) {
  var el = new rfo();
  var sel = this.element.querySelectorAll(selector);
  el.selector = selector;
  el.element = el.el = sel[0];
  el.elements = el.elmts = el.els = sel;
  for(var i=0; i<sel.length; i++) {
    if(sel[i].getAttribute("id")) {
      el.objects.push(_("#" + sel[i].getAttribute("id")));
    }
  }
  return el;
};
